const urlParams = new URLSearchParams(window.location.search);
const roomTypeId = urlParams.get('room');
document.addEventListener('DOMContentLoaded', function() {
    
    fetch('/data/data.json')
    .then(response => {
        // Kiểm tra nếu phản hồi thành công
        if (!response.ok) {
            throw new Error('Network response was not ok ' + response.statusText);
        }
        // Chuyển đổi phản hồi thành JSON
        return response.json();
    })
    .then(data => {
        // Gán dữ liệu vào biến
        listRoomType = data.roomType;
        // Hiển thị dữ liệu trong trang
        var room= listRoomType.find(ele => ele.id == roomTypeId)
        const roomInfor = document.getElementById('room-infor'); 
        roomInfor.innerHTML = `
        <div  class="row d-flex align-items-center">
                <div class="col-6">
                    <img src=${room.imageSrc} alt="" class="" style="width: 100% ; height: 100%;">
                </div>
                <div class="col-6 infor-content">
                    <p class="roomType">${room.name}</p>
                    <p class="capacity">
                        <span class="">
                            <svg fill="#9e9e9e" width="30px" height="30px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" stroke="#9e9e9e"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M23.313 26.102l-6.296-3.488c2.34-1.841 2.976-5.459 2.976-7.488v-4.223c0-2.796-3.715-5.91-7.447-5.91-3.73 0-7.544 3.114-7.544 5.91v4.223c0 1.845 0.78 5.576 3.144 7.472l-6.458 3.503s-1.688 0.752-1.688 1.689v2.534c0 0.933 0.757 1.689 1.688 1.689h21.625c0.931 0 1.688-0.757 1.688-1.689v-2.534c0-0.994-1.689-1.689-1.689-1.689zM23.001 30.015h-21.001v-1.788c0.143-0.105 0.344-0.226 0.502-0.298 0.047-0.021 0.094-0.044 0.139-0.070l6.459-3.503c0.589-0.32 0.979-0.912 1.039-1.579s-0.219-1.32-0.741-1.739c-1.677-1.345-2.396-4.322-2.396-5.911v-4.223c0-1.437 2.708-3.91 5.544-3.91 2.889 0 5.447 2.44 5.447 3.91v4.223c0 1.566-0.486 4.557-2.212 5.915-0.528 0.416-0.813 1.070-0.757 1.739s0.446 1.267 1.035 1.589l6.296 3.488c0.055 0.030 0.126 0.063 0.184 0.089 0.148 0.063 0.329 0.167 0.462 0.259v1.809zM30.312 21.123l-6.39-3.488c2.34-1.841 3.070-5.459 3.070-7.488v-4.223c0-2.796-3.808-5.941-7.54-5.941-2.425 0-4.904 1.319-6.347 3.007 0.823 0.051 1.73 0.052 2.514 0.302 1.054-0.821 2.386-1.308 3.833-1.308 2.889 0 5.54 2.47 5.54 3.941v4.223c0 1.566-0.58 4.557-2.305 5.915-0.529 0.416-0.813 1.070-0.757 1.739 0.056 0.67 0.445 1.267 1.035 1.589l6.39 3.488c0.055 0.030 0.126 0.063 0.184 0.089 0.148 0.063 0.329 0.167 0.462 0.259v1.779h-4.037c0.61 0.46 0.794 1.118 1.031 2h3.319c0.931 0 1.688-0.757 1.688-1.689v-2.503c-0.001-0.995-1.689-1.691-1.689-1.691z"></path> </g></svg>
                        </span>
                        <span class="">${room.capacityAdult} người lớn - ${room.capacityChildren} trẻ em</span>
                    </p>
                    <p class="price">
                        ${room.price} VND/đêm
                    </p>
                </div>
            </div>
        `;
       
    })
    .catch(error => {
        // Xử lý lỗi nếu có
        console.error('There was a problem with the fetch operation:', error);
    });
    
    
    //Tính toán ngày hiện tại
    var today = new Date();
    var dd = String(today.getDate()).padStart(2, '0');
    var mm = String(today.getMonth() + 1).padStart(2, '0'); // Tháng bắt đầu từ 0
    var yyyy = today.getFullYear();

    today = yyyy + '-' + mm + '-' + dd;

    // Thiết lập thuộc tính min là ngày hiện tại cho các trường date để không chọn ngày trong quá khứ
    document.getElementById('check-in').setAttribute('min', today);
    document.getElementById('check-out').setAttribute('min', today);

    const checkInInput = document.getElementById('check-in');
    const checkOutInput = document.getElementById('check-out');
    
    checkInInput.addEventListener('input', validateDates);
    checkOutInput.addEventListener('input', validateDates);
    const bookingForm = document.getElementById('booking-form');
    bookingForm.addEventListener('submit', function(event) {
        event.preventDefault();
        if (validateForm()) bookRoom();
    });
})

//Kiểm tra form booking
function validateForm() {
    const customerName = document.getElementById('customer-name').value;
    const phone = document.getElementById('phone').value;
    const email = document.getElementById('email').value;
    const checkInDate = document.getElementById('check-in').value;
    const checkOutDate = document.getElementById('check-out').value;
  
    if (!customerName) {
        alert('Xin vui lòng nhập họ tên.');
        return false;
    }
  
    if (email && !validateEmail(email)) {
        alert('Xin vui lòng nhập đúng định dạng email.');
        return false;
    }
  
    if (!validatePhone(phone)) {
        alert('Xin vui lòng nhập đúng định dạng số điện thoại.');
        return false;
    }
  
    if (!checkInDate) {
        alert('Xin vui lòng chọn ngày nhận phòng.');
        return false;
    }
  
    if (!checkOutDate) {
        alert('Xin vui lòng chọn ngày trả phòng.');
        return false;
    }
  
    if (new Date(checkInDate) >= new Date(checkOutDate)) {
        alert('Ngày trả phòng phải lớn hơn ngày nhận phòng.');
        return false;
    }
  
    return true;
}
  // Kiểm tra email
  function validateEmail(email) {
    const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return re.test(email);
  }
  
  // Kiểm tra số điện thoại
  function validatePhone(phone) {
    const re = /^\d{10}$/;
    return re.test(phone);
  }
  
  // Kiểm tra ngày check-in check-out
  function validateDates() {
    const checkInDate = document.getElementById('check-in');
    const checkOutDate = document.getElementById('check-out');
    
  
    if (checkInDate.value && checkOutDate.value && new Date(checkInDate.value) >= new Date(checkOutDate.value)) {
        checkOutDate.classList.add('invalid-input');
        checkInDate.classList.add('invalid-input');
    } else {
        checkOutDate.className='';
        checkInDate.className='';
    }
  }
  
  //Đặt phòng
  function bookRoom() {
    // Lấy dữ liệu từ các input
    const customerName = document.getElementById('customer-name').value;
    const phone = document.getElementById('phone').value;
    const email = document.getElementById('email').value;
    const checkInDate = document.getElementById('check-in').value;
    const checkOutDate = document.getElementById('check-out').value;
  
    const booking = {
        customerName,
        phone,
        email,
        checkInDate,
        checkOutDate,
        roomTypeId
    };
    //Thêm vào mảng booking
    bookings.push(booking);
    console.log(bookings)
    const formContainer = document.getElementById('booking-form');
    const successNotification = document.getElementById('submit-success')
    successNotification.style.display = 'flex'
    formContainer.classList.add('d-none');
    document.getElementById('form').reset();
    
  }